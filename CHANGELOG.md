...
- Renamed and Moved the `scripts/setup-env.sh` from scripts to root with new name `setup.sh` for better accessibility. Made respective changes in the `README.md` and `docs/source/quickstart.md`.
- Renamed script `scripts/make_composite_potentials.py` to `scripts/build_potential.py`, updated it to use `_cli_common.py` for argparse.
